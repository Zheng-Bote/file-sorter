...
void FileSorter::sortDownloads(const QList<Category> &categories) {
  ...
  // Lokale Kopie erstellen, um Instabilität bei gleichzeitigen UI-Updates zu
  // vermeiden.
  ...
  // 2. Überwachung pausieren, um Endlosschleifen durch Verschiebevorgänge zu
  // verhindern.
  ...
  for (const auto &fileInfo : files) {
    ...
    for (const auto &cat : localCategories) {
      if (cat.extensions.contains(extension, Qt::CaseInsensitive)) {
        ...
        // Sobald die Datei einer Kategorie zugeordnet wurde, nächste Datei
        // prüfen.
        break;
      }
    }
